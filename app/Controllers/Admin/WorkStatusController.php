...
class WorkStatusController extends BaseController {
	...
	public function __construct()
    {
        ...
        $this->data = array(
            ...
            'session'               => $session,
            'title'                 => 'Work Status',
            'controller_route'      => 'work-status',
            'controller'            => 'WorkStatusController',
            'table_name'            => 'work_status',
            'primary_key'           => 'id'
        );
    }
    public function list()
    {
        if(!$this->common_model->checkModuleFunctionAccess(30, 82)){
    ...
    public function add()
    {
        if(!$this->common_model->checkModuleFunctionAccess(30, 83)){
            $data['action']             = 'Access Forbidden';
            $title                      = $data['action'].' '.$this->data['title'];
            $page_name                  = 'access-forbidden';        
            echo $this->layout_after_login($title,$page_name,$data);
        ...
        $data['action']             = 'Add';
        $title                      = $data['action'].' '.$this->data['title'];
        $page_name                  = 'work-status/add-edit';        
        $data['row']                = [];
    ...
    {
        if(!$this->common_model->checkModuleFunctionAccess(30, 84)){
            $data['action']             = 'Access Forbidden';
            $title                      = $data['action'].' '.$this->data['title'];
            $page_name                  = 'access-forbidden';        
            echo $this->layout_after_login($title,$page_name,$data);
    ...
    {
        if(!$this->common_model->checkModuleFunctionAccess(30, 85)){
            $data['action']             = 'Access Forbidden';
            $title                      = $data['action'].' '.$this->data['title'];
            $page_name                  = 'access-forbidden';        
            echo $this->layout_after_login($title,$page_name,$data);
    ...
    {
        if(!$this->common_model->checkModuleFunctionAccess(30, 86)){
            $data['action']             = 'Access Forbidden';
            $title                      = $data['action'].' '.$this->data['title'];
            $page_name                  = 'access-forbidden';        
            echo $this->layout_after_login($title,$page_name,$data);
